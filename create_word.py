...
class SmartDocumentGenerator:
    ...
    def add_caption(self, text):
        ...
        for m in re.finditer(pattern, clean):
            if m.start() > last_end:
                parts.append({'text': clean[last_end:m.start()], 'bold': False})
            parts.append({'text': m.group(1), 'bold': True})
            last_end = m.end()
        if last_end < len(clean):
            parts.append({'text': clean[last_end:], 'bold': False})

        p = self.doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        self._set_rtl(p)
        for part in parts:
            run = p.add_run(self.text_processor.clean_text(part['text']))
            run.bold = part['bold']
            size_pt = 13
            run.font.name = 'B Nazanin'
        ...
        for i, row_data in enumerate(rows):
            ...
            for j, cell_data in enumerate(row_data):
                ...
                for part in parts:
                    ...
                    txt = re.sub(r'^\*\*|\*\*$', '', part)
                    run = p.add_run(self.text_processor.clean_text(txt))
                    rPr = run._element.get_or_add_rPr()
                    rFonts = rPr.get_or_add_rFonts()
...
def generate_word():
    try:
        ...
        return jsonify({'error': f'خطا در تولید سند ⛔ {str(e)}'}), 200

@app.route('/')
def home():
    return jsonify({'message': 'Persian DOCX Generator ✅'})
